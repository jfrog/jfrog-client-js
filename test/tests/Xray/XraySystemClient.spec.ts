...
import { IClientConfig, IProxyConfig, IXrayVersion } from '../../../model';
...
describe('Xray System tests', () => {
    const clientConfig: IClientConfig = TestUtils.getXrayClientConfig();
    beforeAll(() => {
    ...
    describe('Ping tests', () => {
        ...
        test('Ping failure', async () => {
            ...
            const scope = nock(SERVER_URL).get(`/api/v1/system/ping`).reply(402, { message: 'error' });
    ...
    test('Version', async () => {
        const version: IXrayVersion = await xrayClient.system().version();
